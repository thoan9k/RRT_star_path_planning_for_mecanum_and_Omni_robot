...
class RT_RRT_star:
    def __init__(self, agent, goal, obstacles, mapdimentions, kmax=5, k=100, rs = 10, rb=30, ro=180, rg=36):
        self.agent = agent
        self.goal = goal
        self.old_goal = None
    ...
    def node_in_grid(self, pos):  # Trả về các node trong một vị trí grid
        nodes = [i for i in range(self.number_of_nodes()) if self.grid_pos[i] == pos]
    ...
    def get_XSI(self, Xu): # Xu là 1 node bất kỳ
        m = None
        p = None
        if isinstance(Xu, (list, tuple)): # Nếu Xu là toạ độ
            p = self.goal_grid(Xu)
        elif isinstance(Xu, (int, float)):
            m = Xu
            p = self.grid_pos[m]
        nodes = []
    ...
    def Rewire_random_node(self): # node có sẵn trong hàng đợi Qr (xrand hoặc xclosest )
        ...
        elapse_time = 0
        while(elapse_time < 0.01 and self.Qr):
            xr = self.Qr.pop(0)
    ...
    def sample(self, alpha=0.1, beta=2): # trả về node 
        ...
        if not self.goal:
            ...
            if self.path:
                while is_point_on_path((self.x[xrand], self.y[xrand]), self.path):
                    xrand = self.sample_envir()
        elif Pr > 1 - alpha:  #  random in the line to goal 
            xnear = self.FindNodesNear(self.goal)
            xclosest = xnear[0]
...
#
    ...
    def Nodes_in_goalarea(self):
        near_node = self.FindNodesNear(self.goal)
        # if self.goalstate:
    ...
    def Update_tree(self):
        ...
        # Vẽ chướng ngại vật
        for d_ob in self.dynamic_obstacles:
           pygame.draw.circle(self. surface, (0,255,0),(d_ob[1], d_ob[0]), self.rb,1)
           pygame.draw.circle(self. surface, (0,255,0),(d_ob[1], d_ob[0]), self.rb/2,0)
           pygame.display.update()
        pygame.display.update()
...
if __name__ == '__main__':
    ...
    # *
    rt_rrt_star.dynamic_obstacles = [(366, 346), (468, 130), (504, 460)]
    #*
